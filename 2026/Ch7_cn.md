...
> 圆环绕环上一点摆动——需要绕该点的转动惯量。平行轴定理：$I = I_{\text{cm}} + Md^2$。

**解答**：

**(a)** 圆环绕质心的转动惯量 $I_{\text{cm}} = MR^2$（所有质量在距质心 $R$ 处）。

绕环上一点：$I = I_{\text{cm}} + MR^2 = MR^2 + MR^2 = 2MR^2$

质心到转轴距离 $d = R$

**(b)** 
$$
T = 2\pi\sqrt{\frac{I}{Mgd}} = 2\pi\sqrt{\frac{2MR^2}{MgR}} = 2\pi\sqrt{\frac{2R}{g}}
$$

代入数值：$T = 2\pi\sqrt{\dfrac{0.60}{9.8}} \approx \mathbf{1.55\ \text{s}}$

**(c)** $L_{\text{eq}} = \dfrac{I}{Md} = \dfrac{2MR^2}{MR} = 2R = \mathbf{0.60\ \text{m}}$

圆环绕边缘摆动的等效摆长恰好等于直径——这意味着它的周期与摆长为直径的单摆完全相同。

**(d)** 附加等质量质点于对侧（距转轴 $2R$）：

新总质量 $M' = 2M$
新质心距转轴：$d' = \dfrac{M \cdot R + M \cdot 2R}{2M} = 1.5R$
新转动惯量：$I' = I_{\text{环}} + I_{\text{质点}} = 2MR^2 + M(2R)^2 = 2MR^2 + 4MR^2 = 6MR^2$

$$
T' = 2\pi\sqrt{\frac{6MR^2}{2M \cdot g \cdot 1.5R}} = 2\pi\sqrt{\frac{6R}{3g}} = 2\pi\sqrt{\frac{2R}{g}} = T
$$

周期不变！这是因为 $I'/(M'd') = 6MR^2/(3MR) = 2R$ 恰好等于原来的 $I/(Md) = 2MR^2/(MR) = 2R$。

> 💡 **周期不变的原因**：虽然总质量和转动惯量都增加了，但 $I/(Md)$ 保持不变——这是物理摆设计中的核心洞察：可以通过调整质量分布来控制周期。
...
**(a)** 两根弹簧并联：$k_{\text{eff}} = 2k = 30\ \text{N/m}$

$$
T_1 = 2\pi\sqrt{\frac{m}{k_{\text{eff}}}} = 2\pi\sqrt{\frac{0.20}{30}} \approx \mathbf{0.513\ \text{s}}
$$

**(b)** 
$$
T_2 = 2\pi\sqrt{\frac{L}{g}} = 2\pi\sqrt{\frac{0.50}{9.8}} \approx \mathbf{1.42\ \text{s}}
$$

**(c)** $\omega_1 = \sqrt{k_{\text{eff}}/m} = \sqrt{30/0.20} \approx 12.25\ \text{rad/s}$

$v_{\max} = A\omega_1 = 0.080 \times 12.25 \approx \mathbf{0.98\ \text{m/s}}$

**(d)** $T_1 \neq T_2$（$0.513\ \text{s}$ vs $1.42\ \text{s}$），不同步。

要使 $T_1 = T_2$，需要 $\sqrt{m/k_{\text{eff}}} = \sqrt{L/g}$，即：
$$
\frac{m}{k_{\text{eff}}} = \frac{L}{g} \quad \Rightarrow \quad k_{\text{eff}} = \frac{mg}{L} = \frac{0.20 \times 9.8}{0.50} \approx 3.92\ \text{N/m}
$$

每根弹簧需 $k = k_{\text{eff}}/2 \approx 1.96\ \text{N/m}$——比原来的 $15\ \text{N/m}$ 软得多。

> 💡 使弹簧振子和单摆同步是经典力学中的一个有趣问题——它展示了两种不同 SHM 系统如何通过参数调整达到共振条件。
