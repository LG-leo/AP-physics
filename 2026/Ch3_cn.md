...
沿斜面方向：重力分量 $mg\sin\theta$（沿斜面向下）、动摩擦力 $f_k$（始终与运动方向相反）

法向方向：$N = mg\cos\theta$

动摩擦力：$f_k = \mu_k N = \mu_k mg\cos\theta = 0.25 \times 30 \times 0.8 = 6\ \text{N}$

**(a)** 物体向上滑行：摩擦力沿斜面向下，合外力沿斜面向下

$$
F_{net} = -(mg\sin\theta + f_k) = -(30 \times 0.6 + 6) = -(18 + 6) = -24\ \text{N}
$$

由动能定理（$v_f = 0$，设向上滑行距离为 $d$）：

$$
W_{net} = -24 \times d = 0 - \frac{1}{2} \times 3 \times 8^2 = -96
$$
...
**(b)** 物体滑回底端：重力分量沿斜面向下（与运动同向，做正功），摩擦力仍与运动反向（做负功）
...
> ⭐ **关键对比**：
> - 上滑时重力和摩擦力都做负功 → 速度从 $8\ \text{m/s}$ 减到 $0$
> - 下滑时重力做正功、摩擦力仍做负功 → 末速度 $5.66\ \text{m/s} < 8\ \text{m/s}$（小于初速度）
> - 摩擦力始终做负功，来回都消耗机械能，所以末速度小于初速度！
...
> 🔗 **$U_g \leftrightarrow K$ 连接分析**：
> - 本题使用 🅰️ **机械能守恒视角**：$mgL(1-\cos\theta) = \frac{1}{2}mv^2$
> - 结果 $v = \sqrt{2gL(1-\cos\theta)}$ 只与摆长 $L$ 和角度 $\theta$ 有关，与质量无关
> - 这是单摆的「$v = \sqrt{2gh}$ 变体」——将高度 $h$ 用 $L$ 和 $\theta$ 表示：$h = L(1-\cos\theta)$
> - AP 常考变化：如果 $\theta = 90^\circ$，则 $h = L$，$v = \sqrt{2gL}$；如果 $\theta = 180^\circ$（摆球在最高点），$h = 2L$，$v = 2\sqrt{gL}$
...
**(b)** 由 $P = Fv$ 且 $F = ma = m\dfrac{dv}{dt}$：

$$
P = mv\frac{dv}{dt} \quad\Rightarrow\quad mv\ dv = P\ dt
$$

两边积分（从 $v = 0$ 到 $v = 30\ \text{m/s}$）：
...
$$
540000 = 90000 \cdot t \quad\Rightarrow\quad t = \mathbf{6\ s}
$$

> 💡 **恒定功率运动的特点**：
> - 速度较小时，牵引力很大，加速度很大
> - 随着速度增大，牵引力减小（因为 $F = P/v$），加速度逐渐减小
> - 速度-时间关系为 $v = \sqrt{2Pt/m}$，不是匀加速！
>
> 这与恒定牵引力的运动完全不同——恒定功率时加速度逐渐减小，恒定力时加速度不变。
...
> - **AP 综合考点**：能量守恒给出速度 $\rightarrow$ 牛顿第二定律给出力 $\rightarrow$ 两个物理定律的无缝衔接
...
> 🔗 **$U_g \\leftrightarrow K$ 连接分析**（AP C 万有引力版）：
> - 使用 🅰️ 机械能守恒，但势能是万有引力形式：$U(r) = -\\dfrac{GMm}{r}$ 而非 $U = mgh$
> - 逃逸条件 $E_{total} \\ge 0$ 的物理含义：
>   - $E_{total} < 0$：动能不足以克服引力束缚 → 物体被「困在」引力势能井中（椭圆轨道）
>   - $E_{total} = 0$：刚好逃逸 → 速度 = $v_{esc}$（抛物线轨道）
>   - $E_{total} > 0$：可以逃逸且还有剩余速度 → 速度 > $v_{esc}$（双曲线轨道）
> - 近地表的 $U = mgh$ 是 $U(r) = -\\dfrac{GMm}{r}$ 的一阶近似——当 $h \\ll R$ 时两者等价，当 $h$ 较大时必须用万有引力形式
> - ⚡ **AP C 核心区分**：近地表用 $mgh$（常数 $g$），高空用 $-GMm/r$（$g$ 随高度变化）——判断标准是 $h$ 是否与 $R$ 可比
