...
飞轮储能（Flywheel Energy Storage）是一种通过旋转质量来储存动能的技术。它的原理很简单：用电动机加速飞轮到高速旋转——能量以转动动能的形式储存；需要时，飞轮带动发电机发电——转动动能转化为电能。

**飞轮储能的优势：**
- **充放电速度快**：比化学电池快得多（秒级 vs 小时级）
- **寿命长**：数十万次充放电循环，几乎无衰减
- **功率密度高**：短时间内可释放巨大功率

**这道题会带你走完一个完整的飞轮储能计算：从飞轮能存多少能量，到释放一部分能量后转速还剩多少。**
...
实心圆柱绕中心轴的转动惯量公式：$I = \frac{1}{2}MR^2$。

$$I = \frac{1}{2} \times 100 \times (0.50)^2 = 50 \times 0.25 = \mathbf{12.5\ \text{kg·m}^2}$$

**这个 $I$ 值意味着什么？** 作为参考，一个 $70\ \text{kg}$ 的人距轴 $1\ \text{m}$ 时的转动惯量是 $70\ \text{kg·m}^2$——比这个飞轮大得多。但飞轮的优势在于它可以高速旋转（$\omega$ 大），从而获得巨大的转动动能。

---

**(b) 求转动动能——飞轮存储了多少能量？**

**第一步：把转速从 rpm 转换为 rad/s。**

$3000\ \text{rpm}$ 的意思是每分钟转 $3000$ 圈。转换为弧度每秒：

$$\omega_0 = 3000\ \frac{\text{rev}}{\text{min}} \times \frac{2\pi\ \text{rad}}{1\ \text{rev}} \times \frac{1\ \text{min}}{60\ \text{s}} = 3000 \times \frac{2\pi}{60} = 100\pi \approx 314.2\ \text{rad/s}$$

**$3000\ \text{rpm}$ 有多快？** 你家汽车的发动机怠速时约 $800\ \text{rpm}$，正常行驶约 $2000-3000\ \text{rpm}$——所以这个飞轮的转速相当于汽车发动机中高速运转的速度。

**第二步：计算转动动能。**

$$K_0 = \frac{1}{2}I\omega_0^2 = \frac{1}{2} \times 12.5 \times (314.2)^2$$

先算 $\omega_0^2$：$314.2^2 \approx 98726$

$$K_0 = 6.25 \times 98726 \approx \mathbf{6.17 \times 10^5\ \text{J}}$$

**这个能量有多大？** $6.17 \times 10^5\ \text{J} \approx 0.171\ \text{kWh}$——足够：
- 一只 $10\ \text{W}$ 的 LED 灯泡点亮约 $17$ 小时
- 一台冰箱运行约 $1-2$ 小时
- 一辆特斯拉 Model S 行驶约 $1\ \text{km}$

对于一台重 $100\ \text{kg}$ 的飞轮来说，这个储能并不算很大——这说明了飞轮储能的局限：**能量密度相对较低**。现代飞轮用碳纤维材料以数万 rpm 旋转，能量密度可以大大提高。

---

**(c) 供能后转速会降多少？**

需要提供 $1.0 \times 10^5\ \text{J}$ 的能量——相当点亮一个 $10\ \text{W}$ 灯泡约 $3$ 小时。

**释放能量后的剩余动能：**

$$K_f = K_0 - 1.0 \times 10^5 = 6.17 \times 10^5 - 1.0 \times 10^5 = 5.17 \times 10^5\ \text{J}$$

**由剩余动能反推角速度：**

$$K_f = \frac{1}{2}I\omega_f^2 \quad \Rightarrow \quad \omega_f = \sqrt{\frac{2K_f}{I}}$$

$$\omega_f = \sqrt{\frac{2 \times 5.17 \times 10^5}{12.5}} = \sqrt{\frac{1.034 \times 10^6}{12.5}} = \sqrt{82720} \approx \mathbf{287.6\ \text{rad/s}}$$

在 rpm 中：$287.6 \times \frac{60}{2\pi} \approx \mathbf{2746\ \text{rpm}}$

**对比：** 初始 $3000\ \text{rpm}$ → 供电后 $2746\ \text{rpm}$——转速只下降了约 $8.5\%$，但释放了 $1.0 \times 10^5\ \text{J}$ 的能量！

**为什么转速下降不多？** 因为 $K \propto \omega^2$——动能与角速度的平方成正比。这意味着飞轮在高转速时储存了大量能量，释放一小部分能量只需转速略微下降。**这也是飞轮储能的优势之一：可以在转速变化不大的情况下输出大量能量。**

> 💡 **飞轮储能的实际意义**：$6.17 \times 10^5\ \text{J}$ 约等于 $0.17\ \text{kWh}$——足够一只 10W 的 LED 灯泡点亮约 17 小时。现代飞轮储能系统使用碳纤维飞轮以数万 rpm 旋转，能量密度可与化学电池媲美。与化学电池相比，飞轮的**功率密度**（单位时间可释放的能量）高得多，适用于需要短时间大功率输出的场景（如电网频率调节、赛车混动系统）。

---

#### 6.1.4 🧪 例题 2：转动动能与平动动能的比较——滚动的球能量去哪了？
...
当你从斜面顶端释放一个球，它会滚下来——但它到达底部的速度比一个同样从斜面滑下来的冰块要**慢**。为什么？

因为球的重力势能转化为**两种**动能：平动动能（整体向前移动）+ 转动动能（绕质心旋转）。而冰块（假设无摩擦滑动）只产生平动动能。所以同样高度释放，球分到平动部分的能量更少——因此速度更慢。

**无滑动滚动的关键条件：$v = R\omega$**——球每转一圈，球心向前移动一个周长。这个条件将平动和转动耦合在一起。
...
滚动球的总动能 = 平动动能 + 转动动能。利用 $v = R\omega$（无滑动条件，$\omega = v/R$）：

$$K_{\text{total}} = \frac{1}{2}Mv^2 + \frac{1}{2}I\omega^2$$

代入实心球的转动惯量 $I = \frac{2}{5}MR^2$：

$$K_{\text{total}} = \frac{1}{2}Mv^2 + \frac{1}{2} \cdot \frac{2}{5}MR^2 \cdot \left(\frac{v}{R}\right)^2$$

第二项化简：$\frac{1}{2} \cdot \frac{2}{5} \cdot Mv^2 = \frac{1}{5}Mv^2$

$$K_{\text{total}} = \frac{1}{2}Mv^2 + \frac{1}{5}Mv^2 = \frac{5}{10}Mv^2 + \frac{2}{10}Mv^2 = \frac{7}{10}Mv^2$$
...
**结果解读：** 实心球滚动时，约 $71\%$ 的能量用于前进（平动），约 $29\%$ 用于旋转（转动）。

**不同形状物体的能量分配对比：**

| 物体 | $I$ | $K_{\text{trans}}$ 占比 | $K_{\text{rot}}$ 占比 |
|:--|:--|:--:|:--:|
| 实心球 | $\frac{2}{5}MR^2$ | $5/7 \approx 71\%$ | $2/7 \approx 29\%$ |
| 实心圆柱 | $\frac{1}{2}MR^2$ | $2/3 \approx 67\%$ | $1/3 \approx 33\%$ |
| 空心球壳 | $\frac{2}{3}MR^2$ | $3/5 = 60\%$ | $2/5 = 40\%$ |
| 空心圆筒 | $MR^2$ | $1/2 = 50\%$ | $1/2 = 50\%$ |

**质量越集中在边缘 → $I$ 越大 → 转动动能占比越大 → 平动占比越小 → 滚动越慢。**
...
$$Mgh = K_{\text{total}} = \frac{7}{10}Mv^2$$

注意 $M$ 和 $g$ 在两边都有，可以约掉——**末速度与质量无关！**

$$v = \sqrt{\frac{10}{7}gh}$$

**对比纯滑动（无摩擦斜面）：**
- 纯滑动：$v_{\text{slide}} = \sqrt{2gh}$
- 纯滚动：$v_{\text{roll}} = \sqrt{\frac{10}{7}gh}$

$$\frac{v_{\text{roll}}}{v_{\text{slide}}} = \sqrt{\frac{10/7}{2}} = \sqrt{\frac{5}{7}} \approx 0.845$$

**滚动球的速度大约是纯滑动物体的 $84.5\%$——慢了约 $15.5\%$。**

> 💡 **核心结论：** 滚动比滑动慢，因为一部分重力势能转化为了转动动能而非全部转化为平动动能。质量分布越靠近边缘（$I$ 越大），转动动能占比越高，滚动越慢。这就是为什么空心球比实心球滚得慢、为什么赛车用轻量化轮毂（减小 $I$ 来提高加速性能）。

---

#### 6.1.5 🧪 例题 3：两个滑轮的转动动能比较——质量分布的影响
...
两个滑轮重量一样、大小一样，但质量分布完全不同：

1. **实心圆盘**：质量均匀分布在整个圆盘上——有些质量靠近轴，有些远离轴
2. **圆环/自行车轮**：所有质量集中在边缘——距离轴最远

**同样的质量，不同的分布 → 不同的转动惯量 → 不同的转动动能。**

**直觉理解：** 想象你推动两个相同的旋转门——一个门的重量集中在门的边缘（靠近门把手），另一个门的重量集中在门的转轴附近。显然，推动边缘重的门需要更多的力气——因为质量远离转轴，更难加速旋转。这就是转动惯量的本质：**质量分布越靠近边缘，转动惯量越大，加速越难。**
...
$$I_{\text{disk}} = \frac{1}{2}MR^2 = \frac{1}{2} \times 5.0 \times (0.20)^2 = 2.5 \times 0.04 = 0.10\ \text{kg·m}^2$$

$$K_{\text{disk}} = \frac{1}{2}I_{\text{disk}}\omega^2 = \frac{1}{2} \times 0.10 \times (30)^2 = 0.05 \times 900 = \mathbf{45\ \text{J}}$$

**自行车轮式（圆环）：**

$$I_{\text{ring}} = MR^2 = 5.0 \times (0.20)^2 = 5.0 \times 0.04 = 0.20\ \text{kg·m}^2$$

$$K_{\text{ring}} = \frac{1}{2}I_{\text{ring}}\omega^2 = \frac{1}{2} \times 0.20 \times 900 = 0.10 \times 900 = \mathbf{90\ \text{J}}$$

**对比：** 圆环的转动动能是圆盘的 **2 倍**！尽管两者质量相同、转速相同、半径相同。

**为什么？** 因为 $I_{\text{ring}} = MR^2$ 是 $I_{\text{disk}} = \frac{1}{2}MR^2$ 的 2 倍——所有质量都在边缘，转动惯量翻倍，转动动能也翻倍。

---

**(b) 加速所需的功：**

根据动能定理，从静止加速到 $\omega$ 所需的功 = 转动动能的变化量：

$$W_{\text{disk}} = \Delta K_{\text{disk}} = 45 - 0 = \mathbf{45\ \text{J}}$$

$$W_{\text{ring}} = \Delta K_{\text{ring}} = 90 - 0 = \mathbf{90\ \text{J}}$$

**圆环需要的功是圆盘的 2 倍！** 要加速圆环到同样的转速，需要做两倍的功。

**反过来看（减速/刹车）：**
- 圆盘储存了 $45\ \text{J}$ 的能量
- 圆环储存了 $90\ \text{J}$ 的能量（同样是 $5\ \text{kg}$、$30\ \text{rad/s}$！）

**工程启示：**

| 场景 | 想要大的 $I$（边缘重） | 想要小的 $I$（质量靠近轴） |
|:--|:--|:--|
| **飞轮储能** | ✅ 储存更多能量 | ❌ 储能少 |
| **汽车车轮** | ❌ 加速/刹车更费力 | ✅ 加速快、刹车灵敏 |
| **陀螺仪** | ✅ 更稳定 | ❌ 容易受干扰 |
| **赛车车轮** | ❌ 响应慢 | ✅ 轻量化轮毂提升加速 |

> 💡 **核心结论：** 同样的质量，同样的半径，同样的转速——质量分布在边缘的物体储存的转动动能是均匀分布物体的 2 倍（当 $I_{\text{ring}} = 2I_{\text{disk}}$ 时）。这就是为什么飞轮设计者喜欢把质量放在边缘（储存更多能量），而赛车工程师喜欢轻量化轮毂（减少转动惯量，加速更快）。**在旋转世界中，质量分布比质量大小本身更重要！**
>
...
#### 6.4.7 🧪 例题 5：旋转木马——人从边缘走向中心（滑冰运动员效应）
...
**先理解物理场景——旋转中的人向内走，会发生什么？**

想象一个巨大的旋转平台（像游乐场的旋转木马），一个人站在边缘。平台以恒定速度旋转。然后这个人开始向中心走——会发生什么？

**直觉：** 平台会**加速旋转**！人越靠近中心，平台转得越快。

**为什么？** 角动量守恒定律说：$L = I\omega$ = 常数（如果没有外力矩）。当人向中心走时，人的质量更靠近转轴，所以人的转动惯量 $mr^2$ 减小 → 系统的总 $I$ 减小 → $\omega$ 必须增大来维持 $L$ 不变。

**这就是花样滑冰运动员收臂加速旋转的原理！** 滑冰运动员旋转时，手臂张开（$I$ 大，$\omega$ 小），收臂（$I$ 小，$\omega$ 大）——角动量不变，角速度增大。

---

**解答：**

**(a) 人走到 $r = 0.50\ \text{m}$ 处时的角速度：**

**先求平台的转动惯量（不变）：**

$$I_{\text{platform}} = \frac{1}{2}MR^2 = \frac{1}{2} \times 200 \times (3.0)^2 = 100 \times 9 = 900\ \text{kg·m}^2$$

**初始状态（人在边缘，$r_i = R = 3.0\ \text{m}$）：**

人的转动惯量：$I_{\text{person,i}} = mR^2 = 60 \times 9 = 540\ \text{kg·m}^2$

总转动惯量：$I_i = 900 + 540 = 1440\ \text{kg·m}^2$

**末状态（人走到距中心 $r_f = 0.50\ \text{m}$）：**

人的转动惯量：$I_{\text{person,f}} = m r_f^2 = 60 \times (0.50)^2 = 60 \times 0.25 = 15\ \text{kg·m}^2$

总转动惯量：$I_f = 900 + 15 = 915\ \text{kg·m}^2$

**应用角动量守恒：$I_i\omega_i = I_f\omega_f$**

$$\omega_f = \omega_i \cdot \frac{I_i}{I_f} = 2.0 \times \frac{1440}{915} \approx \mathbf{3.15\ \text{rad/s}}$$

**结果解读：** 角速度从 $2.0\ \text{rad/s}$ 增加到了 $3.15\ \text{rad/s}$——增大了约 $57.5\%$。人还没走到正中心，转速就已经明显提升了！
...
$$K_i = \frac{1}{2}I_i\omega_i^2 = \frac{1}{2} \times 1440 \times (2.0)^2 = 720 \times 4 = 2880\ \text{J}$$

**末态动能：**

$$K_f = \frac{1}{2}I_f\omega_f^2 = \frac{1}{2} \times 915 \times (3.15)^2 \approx 457.5 \times 9.92 \approx 4540\ \text{J}$$

**动能变化：**

$$\Delta K = K_f - K_i \approx 4540 - 2880 = \mathbf{+1660\ \text{J}}$$

**动能增加了 $1660\ \text{J}$！这能量从哪来的？**

答案是：**人的肌肉做功**。当人向中心走时，离心力（在旋转参考系中）试图把人推向边缘。人要克服这个力向中心走，必须用力——这就像逆风行走一样费力。人肌肉做正功，将化学能转化为机械能，增加了系统的转动动能。

**换个角度（用公式理解）：** $K = \frac{L^2}{2I}$。角动量 $L$ 不变，转动惯量 $I$ 减小 → 分母变小 → $K$ 增大。所以 $K$ 增大是角动量守恒和几何变化共同导致的必然结果。

---

**(c) 人走到正中心（$r = 0$）：**

人在中心时，$r = 0$，人的转动惯量 $mr^2 = 0$。

$$I_f = 900 + 0 = 900\ \text{kg·m}^2$$

$$\omega_f = 2.0 \times \frac{1440}{900} = \mathbf{3.20\ \text{rad/s}}$$

**对比：**
| 位置 | $r$ | $I_{\text{total}}$ | $\omega$ | $K$ |
|:--|:--:|:--:|:--:|:--:|
| 边缘 | $3.0\ \text{m}$ | $1440$ | $2.00\ \text{rad/s}$ | $2880\ \text{J}$ |
| $0.50\ \text{m}$ 处 | $0.50\ \text{m}$ | $915$ | $3.15\ \text{rad/s}$ | $4540\ \text{J}$ |
| 中心 | $0$ | $900$ | $3.20\ \text{rad/s}$ | $4608\ \text{J}$ |

**有趣的事实：** 人从 $r=0.50\ \text{m}$ 走到 $r=0$（正中心），角速度只增加了 $3.20/3.15 \approx 1.6\%$——效果已经很小了。因为平台本身的转动惯量（$900$）远大于人在 $0.50\ \text{m}$ 处的贡献（$15$），人再往里走对 $I$ 的影响微乎其微。

**换句话說：** 如果平台很轻（$M$ 小），人走进的效果会更显著。如果平台很重（$M$ 大），人走进的效果会被"稀释"。

> 💡 **核心启示：** 角动量守恒（$L = I\omega$ 不变）导致 $I$ 减小时 $\omega$ 增大。但 **$L$ 守恒时 $K$ 通常不守恒**——$K = L^2/(2I)$ 随 $I$ 减小而增大。动能增加来源于人的肌肉做功（克服离心力）。这个原理不仅在滑冰中重要——在**中子星坍缩**（半径急剧减小 → 转速暴增）中也起关键作用。
...
**先理解物理场景——这是宇宙中最大的"收臂"动作**

想象一颗像太阳一样巨大的恒星——直径 140 万公里，缓慢地自转（转一圈要 27 天）。当这颗恒星的核燃料耗尽时，它不再有向外的热压力来对抗引力——于是引力"胜利"了，恒星以惊人的速度向中心坍缩。

**坍缩的结果：** 一颗直径仅 $20\ \text{km}$ 的中子星——密度堪比原子核，一茶匙中子星物质重达数十亿吨！

**角动量守恒在这个过程中扮演了什么角色？** 恒星坍缩时没有受到任何外力矩（宇宙空间是真空），所以角动量 $L = I\omega$ 必须守恒。半径缩小了 $7 \times 10^4$ 倍 → 转动惯量 $I \propto R^2$ 缩小了 $(7 \times 10^4)^2$ 倍 → $\omega$ 必须增大同样倍数来补偿。

**这就是宇宙中角动量守恒最壮观的应用——从缓慢旋转的恒星到疯狂旋转的中子星！**

---

**解答：**

**Step 1 — 理解 $I \propto R^2$ 的关系：**

对于球体，$I = \frac{2}{5}MR^2$。质量 $M$ 在坍缩中不变（假设），所以 $I \propto R^2$。

角动量守恒：$I_i\omega_i = I_f\omega_f$

**Step 2 — 求角速度的放大倍数：**

$$\frac{\omega_f}{\omega_i} = \frac{I_i}{I_f} = \frac{R_i^2}{R_f^2}$$

代入数值：

$$\frac{\omega_f}{\omega_i} = \left(\frac{7.0 \times 10^8}{1.0 \times 10^4}\right)^2 = (7.0 \times 10^4)^2 = 4.9 \times 10^9$$

**角速度增大了 $49$ 亿倍！** 这是一个天文数字——任何日常经验都无法比拟的放大倍数。

**Step 3 — 计算末态角速度和周期：**

$$\omega_f = \omega_i \times 4.9 \times 10^9 = 2.7 \times 10^{-6} \times 4.9 \times 10^9 \approx 1.32 \times 10^4\ \text{rad/s}$$

**周期（转一圈所需时间）：**

$$T_f = \frac{2\pi}{\omega_f} = \frac{2\pi}{1.32 \times 10^4} \approx 4.75 \times 10^{-4}\ \text{s}$$

$$T_f \approx \mathbf{0.48\ \text{ms}}$$

---

**结果解读——从 27 天到 0.48 毫秒！**

| 物理量 | 坍缩前（恒星） | 坍缩后（中子星） | 变化倍数 |
|:--|:--:|:--:|:--:|
| 半径 | $7.0 \times 10^8\ \text{m}$ | $1.0 \times 10^4\ \text{m}$ | $\times 1/70000$ |
| 转动惯量 | $I_i$ | $I_f$ | $\times 1/(4.9 \times 10^9)$ |
| 角速度 | $2.7 \times 10^{-6}\ \text{rad/s}$ | $1.32 \times 10^4\ \text{rad/s}$ | $\times 4.9 \times 10^9$ |
| 自转周期 | 27 天 | **0.48 毫秒** | $\times 1/(4.9 \times 10^9)$ |

**中子星每秒旋转约 $2100$ 圈！** 如果你站在中子星表面，赤道速度将达到约 $v = R_f\omega_f = 10^4 \times 1.32 \times 10^4 \approx 1.32 \times 10^8\ \text{m/s} \approx 0.44c$——近一半的光速！

---

**为什么这很重要？——脉冲星的发现**

这种快速旋转的中子星在观测上表现为**脉冲星**——它像宇宙灯塔一样，每转一圈就向地球发射一束射电辐射。

**1967 年，剑桥大学的研究生 Jocelyn Bell Burnell 首次探测到来自太空的极其规律的射电脉冲——周期精确到 $1.337\ \text{s}$。** 这个发现被认为是 20 世纪天文学最重要的发现之一，证明了中子星的存在，并直接获得了 1974 年的诺贝尔物理学奖。

**蟹状星云脉冲星**（PSR B0531+21）自转周期约 $33\ \text{ms}$，是 1054 年中国宋代天文学家记录的超新星爆发的遗迹。它每秒发出 30 次脉冲，精确度堪比原子钟。

> 💡 **核心启示：** 恒星从 27 天一圈变为每秒旋转约 2100 圈！这就是脉冲星——快速旋转的中子星发出极其规律的电磁脉冲，被誉为「宇宙中最精确的时钟」。角动量守恒是天体物理中最强大的工具之一——它让我们能够理解从旋转木马到中子星的一切旋转现象，从日落到超新星爆发，$L = I\omega = \text{常数}$ 这一简单原理始终适用。
...
**先理解物理场景——线轴是"方向决定一切"的经典问题**

线轴问题（也称为"溜溜球"问题）是 AP 物理中**最考验直觉**的题之一。同样拉线轴，从上方拉和从下方拉，结果可能完全不同——摩擦力方向甚至可能相反！

**为什么？** 因为拉力的作用点（内轴）和外轴（与地面接触）不同，拉力对质心的力矩方向取决于拉力方向：
- **绳从上方拉**：拉力对质心产生**顺时针**力矩 → 线轴向后转 → 摩擦力可能向前
- **绳从下方拉**：拉力对质心产生**逆时针**力矩 → 线轴向前转 → 摩擦力可能向后

**解题核心：** 联立平动方程（$F_{\text{net}} = ma$）和转动方程（$\tau_{\text{net}} = I\alpha$），再加纯滚动条件 $a = R\alpha$。
...
转动惯量：$I = \frac{1}{2}MR^2 = \frac{1}{2} \times 0.80 \times 0.12^2 = 0.40 \times 0.0144 = 0.00576\ \text{kg·m}^2$

$MR^2 = 0.80 \times 0.0144 = 0.01152$

$\beta = I/(MR^2) = 0.00576/0.01152 = 0.5$
...
**设摩擦力 $f$ 方向向后（即向右）。为什么这样设？** 拉力 $F$ 向后拉内轴，线轴整体趋向于向后（向右）运动。地面摩擦力会阻碍这种运动——所以摩擦力方向**向前（向左）**来阻止滑动。但为了严谨，我们先假设 $f$ 的方向，计算结果的正负会告诉我们真实方向。

**平动方程（向右为正）：** $F_{\text{net}} = Ma$

$$-F + f = Ma \quad \text{（注意 $F$ 向左，$f$ 假设向右）}$$

等等，F 向左拉（向后拉），而假设摩擦力 f 向右（向前）。如果线轴向右运动（被向后拉），速度方向是向右的。让我重新设定方向。

**设向右为正方向。** 绳从上方向右拉？不对，题目说绳从上方拉——拉力方向通常是水平的，可能向右或向左。典型线轴问题中，绳从上方拉时拉力向右（拉线轴向右走）。

**假设：** 拉力 $F$ 向右，线轴向右运动，质心向右加速。

**平动方程（$+x$ 向右）：**

$$F - f = Ma \quad \text{(1)}$$

其中 $f$ 是摩擦力方向（假设向左，因为线轴向右运动，摩擦力向左阻碍运动）。

**转动方程（顺时针为正，即线轴向右滚动的旋转方向）：**

$$F \cdot r - f \cdot R = I\alpha \quad \text{(2)}$$

$F r$ 是拉力对质心的力矩（$F$ 通过内轴拉动，产生顺时针力矩），$f R$ 是摩擦力对质心的力矩（$f$ 在接触点，产生逆时针力矩，取负号）。

**纯滚动条件：** $a = R\alpha$（质心加速度与角加速度通过半径联系）... (3)

**联立求解（将 $\alpha = a/R$ 代入(2)）：**

$$F r - f R = I \cdot \frac{a}{R}$$

代入 $I = \beta MR^2$：

$$F r - f R = \beta MR^2 \cdot \frac{a}{R} = \beta MR a$$

除以 $R$：

$$F \cdot \frac{r}{R} - f = \beta Ma \quad \text{(2')}$$

**从(1)和(2')消去 $f$：**

从(1)：$f = F - Ma$

代入(2')：$F \cdot \frac{r}{R} - (F - Ma) = \beta Ma$

$$F\left(\frac{r}{R} - 1\right) = Ma(\beta - 1)$$

$$a = \frac{F(\frac{r}{R} - 1)}{M(\beta - 1)}$$

代入数值：$\frac{r}{R} = \frac{0.04}{0.12} = \frac{1}{3}$，$\beta = 0.5$

$$a = \frac{3.0 \times (1/3 - 1)}{0.80 \times (0.5 - 1)} = \frac{3.0 \times (-2/3)}{0.80 \times (-0.5)} = \frac{-2.0}{-0.40} = \mathbf{5.0\ \text{m/s}^2}$$

**求摩擦力：** $f = F - Ma = 3.0 - 0.80 \times 5.0 = 3.0 - 4.0 = -1.0\ \text{N}$

$f < 0$ → 摩擦力方向与假设相反——实际方向**向右**（向前）。

**答案（上方拉）：** $a = 5.0\ \text{m/s}^2$（向右），$f = 1.0\ \text{N}$ 方向向右（向前），✅ 纯滚动。
...
**同样的方程，但 $F$ 在质心下方？** 实际上，绳从下方拉时，$F$ 通过内轴对质心产生的是**逆时针**力矩（与滚动方向相反）。所以转动方程中的力矩符号改变。

假设 $F$ 向右，线轴向右运动。摩擦力 $f$ 向左。

**平动方程（同）：** $F - f = Ma$ ... (1)

**转动方程（逆时针力矩现在为正？还是用与滚动方向一致的符号？）**

线轴向右滚动 → 顺时针为正。$F$ 从下方拉内轴 → 力矩为逆时针 → 取负。
...
$$-F r + f R = I \cdot \frac{a}{R} = \beta MR a$$
...
代入(2')：$-F \cdot \frac{1}{3} + (F - Ma) = 0.5 Ma$

$$F\left(1 - \frac{1}{3}\right) = Ma(0.5 + 1)$$

$$F \cdot \frac{2}{3} = 1.5 Ma$$

$$a = \frac{2F/3}{1.5M} = \frac{2 \times 3.0/3}{1.5 \times 0.80} = \frac{2.0}{1.20} \approx \mathbf{1.67\ \text{m/s}^2}$$

**求摩擦力：** $f = F - Ma = 3.0 - 0.80 \times 1.67 = 3.0 - 1.33 = 1.67\ \text{N}$

$f > 0$ → 摩擦力方向与假设一致——向左（向后）。

---

**两种情形的对比总结：**

| 情形 | 加速度 | 摩擦力大小 | 摩擦力方向 | 直观解释 |
|:--|:--:|:--:|:--|:--|
| **绳从上方拉** | $5.0\ \text{m/s}^2$ | $1.0\ \text{N}$ | **向前**（与运动同向） | 拉力试图让线轴顺时针转太快 → 摩擦力向前推动 |
| **绳从下方拉** | $1.67\ \text{m/s}^2$ | $1.67\ \text{N}$ | **向后**（与运动反向） | 拉力试图让线轴逆时针转 → 摩擦力向后阻碍 |

**结果都是纯滚动！** 但两个情形中摩擦力的方向完全相反——这就是线轴问题的精妙之处：**摩擦力的方向不是由运动方向决定的，而是由拉力作用点的位置和方向决定的。**

> 💡 **核心启示：** 线轴问题完美展示了如何处理联立平动-转动方程。关键在于正确写出力矩的符号（注意拉力对质心的力矩方向），然后与纯滚动条件 $a = R\alpha$ 联立。摩擦力的方向可以从计算结果的正负判断——正值表示与假设同向，负值表示反向。**记住：在纯滚动中，摩擦力的作用是调节平动和转动的耦合，它的方向并不总是与运动方向相反！**
